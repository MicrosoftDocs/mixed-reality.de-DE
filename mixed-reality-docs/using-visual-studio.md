...
2. Erstellen Sie ein neues universelles Windows-App-Projekt in Visual Studio 2015 Update 1, Visual Studio 2017 oder Visual Studio 2019. C#- C++,-und JavaScript-Projekte werden alle unterstützt. (Oder befolgen Sie die Anweisungen zum [Erstellen einer APP in Unity](holograms-100.md).)
...
3. Schauen Sie sich die Kachel " **Einstellungen** " an, und führen Sie die [Tasten](gestures.md#air-tap) Kombination aus. Führen Sie die Zeigefingergeste ein zweites Mal aus, um die App in Ihrer Umgebung zu platzieren. Die Einstellungs-App wird gestartet, nachdem Sie sie platziert haben.
...
7. Optional: Scrollen Sie nach unten, und aktivieren Sie das **Geräte Portal**. Auf diese Weise können Sie auch über einen Webbrowser eine Verbindung mit dem [Windows-Geräte Portal](using-the-windows-device-portal.md) auf Ihren hololens herstellen.
...
1. Auswählen einer **x86** -Buildkonfiguration für ![die x86-Buildkonfiguration ihrer app in Visual Studio](images/x86setting.png)
2. Auswählen des **Remote** Computers im Dropdown Menü ![Bereitstellungs Ziel Remote Computer Bereitstellungs Ziel in Visual Studio](images/remotemachinesetting.png)
3. Wechseln C++ Sie für-und JavaScript-Projekte zu **Project > Eigenschaften > Konfigurations Eigenschaften > Debuggen**. Für C# -Projekte wird automatisch ein Dialogfeld angezeigt, in dem die Verbindung konfiguriert wird.
  ...
  b. Dialogfeld "Authentifizierungsmodus auf **Universelles (unverschlüsseltes Protokoll)** ![Remote Verbindung festlegen" in Visual Studio](images/remotedeploy.png)
4. Wählen Sie **Debuggen > Debugging starten** aus, um die![App bereitzustellen und Debuggen starten ohne Debugging in Visual Studio](images/deploynodebugging.png)
...
1. Auswählen einer **x86** -Buildkonfiguration für ![die x86-Buildkonfiguration ihrer app in Visual Studio](images/x86setting.png)
2. Auswählen des **Geräts** im Dropdown Menü!["Bereitstellungs Ziel" Geräte Bereitstellung in Visual Studio](images/buildsettingsusbdeploy.png)
3. Wählen Sie **Debuggen > Debugging starten** aus, um die![App bereitzustellen und Debuggen starten ohne Debugging in Visual Studio](images/deploynodebugging.png)
...
## <a name="pairing-your-device---hololens-1st-gen"></a>Koppeln Ihres Geräts-hololens (1. Gen)
...
3. Wählen Sie im Dropdown Menü![emulatorziel des Bereitstellungs Ziels in Visual Studio **hololens-Emulator** aus.](images/deployemulator.png)
4. Wählen Sie **Debuggen > Debugging starten** aus, um die![App bereitzustellen und Debuggen starten ohne Debugging in Visual Studio](images/deploynodebugging.png)

## <a name="graphics-debugger"></a>Grafik Debugger
